...
enum CHOICE {MAKEEMPTY, ISEMPTY, SIZE, HEIGHT, QUIT, PRINT, PRINTNODE, LEVELORDER,PREORDER, SETROOT, SETLEFT, SETRIGHT, GOTOROOT, GOLEFT, GORIGHT, ISONRIGHTSIDE, FINDMIN };
...
int main(int argc, char* argv[]) {
	...
	BinaryTree<int> lefttree = NULL;
	BinaryTree<int> righttree = NULL;
	...
	do {
		...
		case GORIGHT:
			...
			break;
		case ISONRIGHTSIDE:
			cout << "Enter value to see if parent exists: ";
			cin  >> value;
			if(tree.isOnRightSide(value))
				cout<<"True"<<endl;
			else 
				cout<<"False"<<endl;
			break;
		case FINDMIN:
			cout<<"Minumum value is: "<<tree.findMin()<<endl;
		break;
	}	
...
CHOICE menu() {
	char choice;
	CHOICE result;
	cout << "(M)akeEmpty (I)sEmpty (S)ize (H)eight setRoo(T) set(L)eftSide set(R)ightSide    gotoroot(1) gole(F)t gori(G)ht (P)rint printN(O)de le(V)elOrder Pr(E)Order (Q)uit:"  
		    " isOnRightSi(D)e findMin(z) "<< endl;
	cin  >> choice;
	switch( choice ) {
	case 'M':
	case 'm':
