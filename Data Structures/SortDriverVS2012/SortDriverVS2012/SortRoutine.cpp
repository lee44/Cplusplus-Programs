...
void SortRoutine::exchange( int array[],const int i,const int j ) 
{
	exchanges++;
	int tmp = array[i];
	array[i] = array[j];
...
bool SortRoutine::lessthan( const int array[],  const int i,  const int j ) 
{
	comparisons++;
	bool result = false;
	if (array[i] < array[j])
...
bool SortRoutine::lessthan( const int value1, const int value2 ) 
{
	comparisons++;
	bool result = false;
	if (value1 < value2)
...
void SortRoutine::sort( int array[], int size ) {

}

string SortRoutine::name() const {
	return( "SortRoutine" );
}
