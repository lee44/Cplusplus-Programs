...
void fillarray( int array[], int size, char * f );
void runsort( int array[], int size, SortRoutine * sr, bool show_results );
void usage();
...
int main(int argc, char* argv[])
{
	...
	if (!sort_provided)		
		strcpy_s( s, 1023, "radixsort" );

	if (file_provided) 
	{
		size = getlength( f );
		dataarray = new int[ size ];
		fillarray( dataarray, size, f );
	}
	else 
	{
		size = 128;
	...
	else if (strcmp(s, "radixsort") == 0)
	{
		...
		runsort( dataarray, size, &rs, show_results );
