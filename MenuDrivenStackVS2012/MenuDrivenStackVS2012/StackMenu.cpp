...
enum CHOICE {MAKEEMPTY, PUSH, ISEMPTY, POP, TOP, TOPANDPOP, QUIT, PRINT, ISPALINDROME };
...
CHOICE menu() {
	char choice;
	CHOICE result;
	cout << "(M)akeEmpty I(s)Empty p(U)sh p(O)p (T)op top(A)ndpop (P)rint (I)sPalindrome (Q)uit: " << endl;
	cin  >> choice;
	switch( choice ) {
	case 'A':
	case 'a':
